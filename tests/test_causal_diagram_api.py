# coding: utf-8

from fastapi.testclient import TestClient


from openapi_server.models.causal_diagram import CausalDiagram  # noqa: F401


def test_causaldiagrams_get(client: TestClient):
    """Test case for causaldiagrams_get

    List all instances of CausalDiagram
    """
    params = [("username", 'username_example'),     ("label", 'label_example'),     ("page", 1),     ("per_page", 100)]
    headers = {
    }
    response = client.request(
        "GET",
        "/causaldiagrams",
        headers=headers,
        params=params,
    )

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_causaldiagrams_id_delete(client: TestClient):
    """Test case for causaldiagrams_id_delete

    Delete an existing CausalDiagram
    """
    params = [("user", 'user_example')]
    headers = {
        "Authorization": "Bearer special-key",
    }
    response = client.request(
        "DELETE",
        "/causaldiagrams/{id}".format(id='id_example'),
        headers=headers,
        params=params,
    )

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_causaldiagrams_id_get(client: TestClient):
    """Test case for causaldiagrams_id_get

    Get a single CausalDiagram by its id
    """
    params = [("username", 'username_example')]
    headers = {
    }
    response = client.request(
        "GET",
        "/causaldiagrams/{id}".format(id='id_example'),
        headers=headers,
        params=params,
    )

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_causaldiagrams_id_put(client: TestClient):
    """Test case for causaldiagrams_id_put

    Update an existing CausalDiagram
    """
    causal_diagram = {"value":{"id":"some_id"}}
    params = [("user", 'user_example')]
    headers = {
        "Authorization": "Bearer special-key",
    }
    response = client.request(
        "PUT",
        "/causaldiagrams/{id}".format(id='id_example'),
        headers=headers,
        json=causal_diagram,
        params=params,
    )

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_causaldiagrams_post(client: TestClient):
    """Test case for causaldiagrams_post

    Create one CausalDiagram
    """
    causal_diagram = {"value":{"id":"some_id"}}
    params = [("user", 'user_example')]
    headers = {
        "Authorization": "Bearer special-key",
    }
    response = client.request(
        "POST",
        "/causaldiagrams",
        headers=headers,
        json=causal_diagram,
        params=params,
    )

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200

